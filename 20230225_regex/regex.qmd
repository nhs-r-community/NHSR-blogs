...
For this particular analysis metrics 02 to 09 use a denominator of patients on GP lists for Coronary Heart Disease (CHD) whilst all the others use a GP population count of over 16 years old. For quickness, it's possible to list out each metric and its corresponding calculation but that does make code less readable, particularly as the metrics included have gaps in the numbering:
...
Translating the string output `r by_list_regex` into English would be a bit like:
